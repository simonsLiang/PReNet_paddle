...
from paddle.vision import  transforms as aug
...
class InferenceEngine(object):
    ...
    def __init__(self, args):
        ...
        # build transforms
        self.transforms = aug.Resize((args.crop_size,args.crop_size))
        # wamrup
    ...
    def postprocess(self, x):
        ...
        """
        x = x[0]
        x = paddle.clip(paddle.to_tensor(x,dtype='float32'), 0., 1.)
        out = np.uint8(255 * x.numpy().squeeze())
        return out
...
def get_args(add_help=True):
    ...
    parser.add_argument("--img-path", default="./data/rain-001.png")
...
def infer_main(args):
    ...
    # postprocess
    prob = inference_engine.postprocess(output)
    ...
    print(f"image_name: {args.img_path},, prob_shape: {prob.shape}")
    return prob


if __name__ == "__main__":
    args = get_args()
    prob = infer_main(args)
